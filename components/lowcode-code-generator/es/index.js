      ...
      if (!firstRoot.fileName && !isValidContainerType(firstRoot)) {
        ...
        containers = schema.componentsTree.map((n) => {
          ...
          const container = {
            ...
            moduleName: ensureValidClassName(changeCase2.pascalCase(subRoot.fileName))
          };
          return container;
        });
      }
...
var pluginFactory2 = () => {
  const plugin = async (pre) => {
    ...
    const componentClassName = ensureValidClassName(
      `${changeCase3.pascalCase(ir.moduleName)}$$${ir.containerType}`
    );
    next.chunks.push({
      type: "string" /* STRING */,
      fileType: "jsx" /* JSX */,
    ...
    if (ir.containerType === "Component") {
      next.chunks.push({
        ...
        content: `static displayName = '${changeCase3.pascalCase(ir.moduleName)}';`,
        linkAfter: [
          CLASS_DEFINE_CHUNK_NAME.Start
        ]
      });
    ...
    if (dataSourceItems.length > 0) {
      ...
      dataSourceItems.forEach((ds) => {
        ...
        if (!(dsType in requestHandlersMap) && dsType !== "custom") {
          const handlerFactoryName = `__$$create${changeCase4.pascal(dsType)}RequestHandler`;
          requestHandlersMap[dsType] = {
            type: "JSExpression",
            value: handlerFactoryName + (dsType === "urlParams" ? "(window.location.search)" : "()")
          };
          const handlerFactoryExportName = `create${changeCase4.pascal(dsType)}Handler`;
          const handlerPkgName = ((_b = (_a = cfg.datasourceConfig) == null ? void 0 : _a.handlersPackages) == null ? void 0 : _b[dsType]) || `@alilc/lowcode-datasource-${changeCase4.kebab(dsType)}-handler`;
          next.chunks.push({
            type: "string" /* STRING */,
            fileType: "jsx" /* JSX */,
            name: COMMON_CHUNK_NAME.ExternalDepsImport,
...
function buildDataSourceDependencies(ir, cfg = {}) {
  ...
  function getDataSourceHandlerPackageName(dsType) {
    var _a;
    return ((_a = cfg.handlersPackages) == null ? void 0 : _a[dsType]) || `@alilc/lowcode-datasource-${changeCase5.kebab(dsType)}-handler`;
  }
}
...
var pluginFactory22 = () => {
  const plugin = async (pre) => {
    ...
    const ir = next.ir;
    const componentClassName = ensureValidClassName(
      `${changeCase6.pascalCase(ir.moduleName)}$$Page`
    );
    next.chunks.push({
      type: "string" /* STRING */,
      fileType: "jsx" /* JSX */,
  ...
  const plugin = async (pre) => {
    ...
    if (dataSourceItems.length > 0) {
      ...
      dataSourceItems.forEach((ds) => {
        ...
        const dsType = ds.type || "fetch";
        if (!(dsType in requestHandlersMap) && dsType !== "custom") {
          const handlerFactoryName = `__$$create${changeCase7.pascal(dsType)}RequestHandler`;
          requestHandlersMap[dsType] = {
            type: "JSExpression",
            value: `${handlerFactoryName}(${dsType === "urlParams" ? "__$$getSearchParams()" : ""})`
          };
          const handlerFactoryExportName = `create${changeCase7.pascal(dsType)}Handler`;
          const handlerPkgName = ((_a3 = cfg.dataSourceHandlersPackageMap) == null ? void 0 : _a3[dsType]) || `@alilc/lowcode-datasource-${changeCase7.kebab(dsType)}-handler`;
          next.chunks.push({
            type: "string" /* STRING */,
            fileType: "jsx" /* JSX */,
            name: COMMON_CHUNK_NAME.ExternalDepsImport,
...
function generateEventHandlerAttrForRax(attrName, attrValue, scope, config) {
  ...
  return [
    ...referencedLocalVariables.map((localVar) => ({
      type: "NodeCodePieceAttr" /* ATTR */,
      name: `data-${changeCase8.snake(localVar)}`,
      value: localVar
    })),
    {
      type: "NodeCodePieceAttr" /* ATTR */,
...
var pluginFactory32 = (cfg) => {
  const plugin = async (pre) => {
    ...
    const routes = ((_b = (_a = ir.globalRouter) == null ? void 0 : _a.routes) == null ? void 0 : _b.map((route) => ({
      path: route.path,
      source: `pages/${ensureValidClassName(changeCase9.pascalCase(route.fileName))}/index`
